      ...
      <DialogContent showCloseButton={false} className="sm:max-w-md rounded-2xl">
        ...
        <div className="space-y-4 py-2">
          ...
          {notes && (
            <p className="text-sm text-[#64748B] px-1">
              <span className="font-medium text-[#475569]">Alasan:</span> {notes}
            </p>
          )}
        </div>
        ...
        <DialogFooter className="flex-row gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className="flex-1 h-12 rounded-xl text-sm font-bold text-white shadow-sm flex items-center justify-center gap-2 active:scale-95 transition-all duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 bg-red-600 hover:bg-red-700 focus-visible:ring-red-500"
          >
            {isLoading ? (
              <><Spinner /> Memproses...</>
            ) : (
